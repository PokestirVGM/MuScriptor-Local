...
Turn recordings into MIDI on your computer, with a desktop app and local web GUI. This adaptation adds tempo and time-signature controls, separate optional quantization, reusable sessions, and early-finish exports to [MuScriptor](https://github.com/muscriptor/muscriptor). The app handles setup, model downloads, and switching between Small, Medium, and Large.

Independent adaptation by **Pokestir**, based on MuScriptor by Kyutai and Mirelo. Audio processing stays on your computer.

**[Download Windows 1.0 RC1](https://github.com/PokestirVGM/MuScriptor-Local/releases/tag/v1.0.0-rc.windows.1)** · **[Download macOS 1.0 RC1](https://github.com/PokestirVGM/MuScriptor-Local/releases/tag/v1.0.0-rc.mac.1)** · [Report an issue](https://github.com/PokestirVGM/MuScriptor-Local/issues) · [Changelog](CHANGELOG.md)

## Current release: 1.0 Release Candidate 1

**RC1 is a prerelease for testing, not stable 1.0.** Both platforms include the adapted local web GUI, portable sessions, recovery, Finish here, timing controls, and export readiness checks. macOS requires Apple Silicon and macOS 14+; Windows requires Windows 10/11 x64. macOS is ad-hoc signed and not notarized; Windows is unsigned. See [Mac installation](MACOS.md), [Windows installation](WINDOWS.md), and the [validation record](validation/FINAL_READINESS.md).

**Save Session** stores the original notes, detected beats, current timing settings, and available source audio in a `.muscriptor` file. Open it in the desktop app or local web GUI on either platform without transcribing again. Sessions can include audio, so only share them when you intend to share the recording. MIDI-only sessions are also supported. The file limit is 256 MB. Desktop sessions embed a decoded mono WAV; browser sessions embed the selected original audio.
...
| Platform | Status |
| --- | --- |
| macOS 14+ on Apple Silicon | [1.0 RC1 — app ZIP](https://github.com/PokestirVGM/MuScriptor-Local/releases/download/v1.0.0-rc.mac.1/MuScriptor-Local-1.0-RC1-macOS-Apple-Silicon.zip) |
| Windows 10/11 x64 | [1.0 RC1 — installer](https://github.com/PokestirVGM/MuScriptor-Local/releases/download/v1.0.0-rc.windows.1/MuScriptor-Local-1.0-RC1-Windows-x64-Setup.exe) · [portable ZIP](https://github.com/PokestirVGM/MuScriptor-Local/releases/download/v1.0.0-rc.windows.1/MuScriptor-Local-1.0-RC1-Windows-x64-Portable.zip) |
| Intel Mac, Windows ARM, Linux desktop | No packaged release |

Windows supports NVIDIA CUDA and **experimental AMD GPU acceleration through DirectML**. RC1 received a real Large-model transcription check on RTX 5070 Ti; full RX 6800 XT RC1 acceptance remains open. After upgrading an existing Windows installation, use **gear menu → Repair Dependencies**. See [AMD setup and validation limits](WINDOWS.md#amd-gpus-directml-experimental).
...
1. Download **MuScriptor-Local-1.0-RC1-macOS-Apple-Silicon.zip** from the [macOS RC1 release](https://github.com/PokestirVGM/MuScriptor-Local/releases/tag/v1.0.0-rc.mac.1).
...
5. Leave Tempo on **Auto**, or enter a known **Manual** BPM and time signature. This does not change playback speed. **Strict quantization** and **Create A/B audio render** are separate, optional choices and start unchecked.
...
Keep the desktop app open. **Return to Desktop** stops the web server and any active web transcription, then restores the latest saved session on desktop. Save/apply browser changes first; unsaved edits are not retained. Only one interface owns the model at a time. Choose transcription options in the interface you are using. Browser-generated files use your browser's download/save location, independently of the desktop MIDI destination.
...
To update, quit the app, replace it with a newer release, and reopen it. The portable app refreshes its local worker automatically. Use **gear menu → Repair Dependencies…** if dependency setup fails or needs refreshing. **Show Logs** opens the diagnostic folder. Review and redact local paths before posting logs publicly.
