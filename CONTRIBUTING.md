...
Keep model inference in the official engine. Local rhythm export lives in the shared engine extension recorded in `patches/upstream-rhythm.patch`; preserve that reproducible patch whenever editing the ignored upstream checkout. Test MIDI note times as well as displayed BPM: changing tempo or meter must never stretch playback or add a start offset, and quantization must remain explicit. Do not add cloud uploads, analytics, model weights, credentials, or personal recordings to the repository or release archives. Preserve upstream notices and separate the code license from the model license.
